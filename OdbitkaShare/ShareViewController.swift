...
/// Wejście z systemowego arkusza udostępniania (Zdjęcia → Udostępnij → Odbitka).
///
/// Zachowanie zależy od wielkości paczki:
/// - **do `OdbitkaConfig.extensionPhotoLimit` zdjęć** rozszerzenie robi wszystko na miejscu:
...
final class ShareViewController: UIViewController {
    ...
    private func prepare() async {
        ...
        if providers.count > OdbitkaConfig.extensionPhotoLimit {
    ...
    private func processInPlace(providers: [NSItemProvider]) async {
        ...
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("OdbitkaShare-\(UUID().uuidString)", isDirectory: true)
